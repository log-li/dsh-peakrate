...
**跨午夜窗口归属「开始日」**（2026-09-12 修正，独立 review 指出）：
`23:00-09:00` 这类跨午夜窗口，其**凌晨段 `[00:00, end)` 归属窗口的*开始日*** ——
日期区间与 `days` 都按开始日判定，与常规窗口的 `isPeakAt`（prev-day spill）语义一致。

原实现按**落点日**过滤，导致两个偏差（已修）：
- 真实 Z.ai 数据（23:00-09:00，09-03~09-20）实际生效成 `09-03 00:00 ~ 09-21 00:00`，
  即**两端各偏一天**：区间首日凌晨被误算作活动，末夜的凌晨段被午夜截断。
  修正后为 `09-03 23:00 ~ 09-21 09:00`。
- `days` 为子集时（如仅周五），跨午夜活动的**后半段静默失效**。

**数据校验约束**（`catalog.ts`，2026-09-12 加固）：
- override 窗口**丢弃 `start === end`** —— 否则跨午夜分支
  `minutes >= start || minutes < end` 恒真 → **24/7 全天生效**。
  （远端 pricing.json 是运行时拉的，属真实坏数据面。）
- `startDate` / `endDate` 必须是 `YYYY-MM-DD`（正则校验）—— schedule 层用字符串比较，
  非此格式会静默错判区间。
