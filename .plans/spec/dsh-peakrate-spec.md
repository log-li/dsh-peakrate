...
### 5.1 载体：**可加性槽位，不遮蔽自带 UI**（2026-09-12 重大修订）
...
**放弃**：模型选择器**每一行**的倍率徽章。该能力必须替换自带选择器才能实现，
与「不遮蔽自带 UI」不可兼得。

**保留**（核心价值）：当前模型的**峰谷状态 + 倍率 + 切换倒计时**——
即「我正在用的这个模型，现在什么价、还有多久切换」，这正是本插件区别于
DeepSeek 专用插件的部分（同一模型经不同 provider 时段规则不同）。

**将来若需**「列出全部匹配模型的倍率」，可另加 `conversation.input.overlay`
（同属 replaceRisk: none）做一个可展开面板，**仍不替换选择器**。
