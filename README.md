...
在 **composer 工具行左侧**（紧邻模型选择器）显示**当前模型**的峰谷状态、倍率与切换倒计时：

| 位置 | 内容 |
|---|---|
| 当前模型已匹配 | `⚡2× · 2h 30m`（峰）/ `🌙1× · 5h`（谷） |
| hover 徽章 | 详情：provider · 模型、当前时段名、峰谷倍率对照、倒计时、核验日期 |
| 当前模型未匹配 | **什么都不显示**（无占位、无灰字） |

倒计时格式：`<1h` 用 `Xm`；`<24h` 用 `Xh Ym`；`≥24h` 用 `Xd Yh`。

### 为什么是「追加」而不是「替换模型选择器」

早期版本用**替换模型选择器**的方式实现（想在每一行都显示倍率徽章），但
`conversation.input.model` 是 `single` + `replaceRisk: shadows-shipped-ui` 槽位
——注册即**遮蔽官方实现**。替换版实现残缺（无 effort 选择、无加载/错误态），
**直接导致无法切换模型**。

现改为注册到 `conversation.input.left`（list · `replaceRisk: none`），用自有
`id` 做**纯追加**，完全不碰自带 UI。代价是**放弃了「选择器内每一行显示徽章」**
——该能力必须替换官方 UI 才能实现，与非破坏性接入不可兼得。

> **设计原则**：宁可少一个显示位置，也不接管用户的核心交互入口。
